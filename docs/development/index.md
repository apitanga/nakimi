---
title: Development
nav_order: 30
has_children: true
---

# Development

This section contains documentation for developers working with or contributing to Nakimi.

## Contents

- [Plugin Development Guide](PLUGIN_DEVELOPMENT.md) - How to create plugins
- [Architecture](ARCHITECTURE.md) - System design and security model
- [Testing Guide](TESTS.md) - Comprehensive testing documentation
- [Architecture Decision Records](ADR.md) - Design decisions and rationale
- [Git Hooks for Test Protection](GIT_HOOKS.md) - Pre-commit and pre-push hooks

## Contributing

If you want to contribute to Nakimi:

1. Read the [Contributing Guide](../../CONTRIBUTING.md)
2. Check the [Development Roadmap](../project/ROADMAP.md) for planned features
3. Review the [Architecture Decision Records](ADR.md) to understand design choices
4. Follow the [Testing Guide](TESTS.md) for quality assurance